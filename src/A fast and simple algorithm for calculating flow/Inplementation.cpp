                ...
                if (Fill[i][j] > DEM[i][j]) {
                    for (const auto& dir : directions) {
                        ...
                        if (newRow >= 0 && newRow < rows 
                            && newCol >= 0 && newCol < cols && DEM[newRow][newCol] != -9999) {
...
std::vector<std::vector<int>> d8_Algorithm(const std::vector<std::vector<float>>& fill) {
    ...
    std::vector<std::vector<int>> dir = { 
        {8, 16, 32}, {4, 0, 64}, {2, 1, 128} };//流出查询表
    std::vector<std::vector<int>> dir_receive = { 
        {128, 1, 2}, {64, 0, 4}, {32, 16, 8} };//流入查询表
            ...
            for (int di = -1; di <= 1; ++di) {
                for (int dj = -1; dj <= 1; ++dj) {
                    ...
                    { 
                        ...
                        // 如果邻居的DEM值与当前单元格相同且邻居的流向为0，则流向指向邻居
                        if (slope < down_slope) {
                            min_dir = dir[di + 1][dj + 1];
                        }
                    }
                        ...
                        float slope = fill[ni][nj] - fill[i][j];
                        // 如果邻居的DEM值与当前单元格相同且邻居的流向不为0，则邻居被指向
                        if (slope < down_slope) {
                            min_dir = dir_receive[di + 1][dj + 1];
                        }
                    }
                    else
                    {
...
Cell NextCell(Cell c, vector<vector<int>>& FlowDir) {
    ...
    for (int k = 0; k < 8; ++k) {
        ...
        if (FlowDir[c.row][c.col] == d8[k]) {
            ...
            // 检查下游单元格是否在 DEM 的范围内
            if (newRow >= 0 && newRow < FlowDir.size() && newCol >= 0 
                && newCol < FlowDir[0].size() && FlowDir[newRow][newCol] != -9999) {
